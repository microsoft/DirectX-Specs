...
> Note: The CSC output is clamped to `[0, 1]` before being passed to the 1DLUT as input, and the 1DLUT output is clamped to `[0, 1]` before being passed to the 3DLUT as input. See more details in the [Data flow](#data-flow) pseudocode section. The normalized range `[0, 1]` maps to the full representable range of the texture format: for `_UNORM` formats this corresponds to the unsigned normalized integer range, and for `_FLOAT` formats this corresponds to the floating-point range from 0.0 to 1.0. For more information about DXGI format ranges, see the [DXGI_FORMAT documentation](https://learn.microsoft.com/en-us/windows/win32/api/dxgiformat/ne-dxgiformat-dxgi_format#format-modifiers).

> Note: If the application is using an input texture with values outside the `[0, 1]` range (for example, `DXGI_FORMAT_R16G16B16A16_FLOAT` with HDR values greater than 1.0), the app **must** use the CSC matrix to scale the input into the `[0, 1]` range to fit within the clamp operation that occurs after the CSC stage. Failure to do so will result in values being clipped at the CSC output clamp, potentially losing important HDR information.
