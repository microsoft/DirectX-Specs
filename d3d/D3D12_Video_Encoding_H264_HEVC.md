# 1. D3D12 Video Encoding H264/HEVC
...
gaps_in_frame_num_value_allowed_flag | 0 | Unless app coding headers for multiple temporal layers. |
...
num_ref_idx_l0_active_minus1 | std::max(static_cast<INT>(pictureControl.List0ReferenceFramesCount) - 1, 0) | None |
num_ref_idx_l1_active_minus1 | std::max(static_cast<INT>(pictureControl.List1ReferenceFramesCount) - 1, 0) | None |
...
vps_base_layer_internal_flag | 1 | None |
vps_base_layer_available_flag | 1 | None |
...
> Note: When operating with temporal SVC layers the VPS/SPS must be authored by the app so that VPS fields (e.g `vps_max_layers_minus1`, `vps_max_layer_id`, etc) and SPS fields (e.g `sps_max_sub_layers_minus1`, etc) reflect the configured temporal layer count and layer ids. The VPS/SPS expectations above apply to a single base layer case; for multi‑layer temporal SVC the VPS/SPS coded by the app must be consistent with the requested temporal layer configuration.
...
Picture layer number in temporal hierarchy. The maximum number of temporal layers is currently 3.
...
If bit set, requests for the driver to write `num_ref_idx_active_override_flag` and the related `num_ref_idx_l0_active_minus1` and `num_ref_idx_l1_active_minus1` in the slice header, overriding the active PPS values for these codec params. Requires `D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264_FLAG_NUM_REF_IDX_ACTIVE_OVERRIDE_FLAG_SLICE_SUPPORT` supported.

In this case the app can reuse the PPS coded for previous frames and avoid a new PPS per frame, as long as the slice override params are enough to specify the bitstream changes, if any.
...
> Note: For H264 the driver **will not** write the nal_ref_idc = 14 prefix NAL indicating the *temporal_id* for the slices coded for this frame when using temporal scalability. The responsability of coding the prefix NAL units falls on the user along with the rest of the bitstream headers such as PPS, SPS, etc.
...
> For any codecs/configurations where the QP ranges can be negative, the ranges used by `pRateControlQPMap` as absolute map parameter are kept in that native signed range.
...
If bit set, requests for the driver to write `num_ref_idx_active_override_flag` and the related `num_ref_idx_l0_active_minus1` and `num_ref_idx_l1_active_minus1` in the slice header, overriding the active PPS values for these codec params. Requires `D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC_FLAG_NUM_REF_IDX_ACTIVE_OVERRIDE_FLAG_SLICE_SUPPORT` supported.
...
> For any codecs/configurations where the QP ranges can be negative, the ranges used by `pRateControlQPMap` as absolute map parameter are kept in that native signed range. For example for HEVC `[0, 51]` range for 8 bit pixel depth, the range for 10 bits `[-12, 51]`, and similar for higher bit depths are all considered as-is from the spec.
...
When requesting an intra-refresh wave for *IntraRefreshDuration* frames by activating *D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_FLAG_REQUEST_INTRA_REFRESH*. This indicates for the current picture the index of the frame in the intra-refresh wave, the value range is set by the host between [0..IntraRefreshDuration) to hint the status of the intra-refresh session to the driver.
...
Starts an intra-refresh session starting at this frame using the configuration in D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_DESC.IntraRefreshConfig. The flag must be kept set for the entire duration of the IR session, disabling it on the non-IR frame.
...
**Remarks about the use of intra-refresh**
    ...
    3) When starting an intra-refresh of N frames of duration, the hosts sets the corresponding picture control flag to start intra-refresh (kept enabled during the entire IR session) and controls the frame index between [0..N) also in the picture control structure (See IntraRefreshFrameIndex) to hint the driver about the progress about the intra refresh session.
...
**Remarks**
    ...
    3) Frames with different `TemporalLayerIndex` can have different properties such as bitrates or QP constants, but cannot have different rate control **types** (e.g VBR/CBR/CQP). When beginning the encoding session, the first EncodeFrame called for each TemporalLayerIndex will contain the initial rate control configuration for that layer without any additional flags set. Any changes to any layer **after** the initial frame for it, must follow the same rules for RC reconfiguration (e.g use D3D12_VIDEO_ENCODER_SUPPORT_FLAG_RATE_CONTROL_RECONFIGURATION_AVAILABLE if supported by driver or re-create the ID3D12VideoEncoder object)
...
### 6.1.20. STRUCT: D3D12_VIDEO_ENCODER_ENCODEFRAME_INPUT_ARGUMENTS
