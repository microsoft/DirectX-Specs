...
Whether the implementation may overlap the next list's launch with the tail of the current one is controlled by [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) (set explicitly, or implied for in-order graphics-class lists):

**No wait, pre-staged or asynchronous chains.** With only [`_ALLOW_NEXT_DISPATCH_LIST_CONTINUATION`](#d3d12_dispatch_list_flags1) set on the current list (and not on an in-order graphics list), the app asserts that the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) (the one this list's `NextDispatchList` points at, and what that one references) is valid by the time the implementation reads it, whether the app wrote it from the CPU or had earlier GPU work produce it (with appropriate barriers before the [`DispatchList1`](#dispatchlist1) call). The implementation may read `NextDispatchList` at any time, including at launch, and is free to overlap the launch of the next list with the tail of the current one for tighter scheduling. Subsequent lists deeper in the chain bring their own continuation contract per their own [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) setting, the chain can mix wait and no-wait lists freely. Typical use case: a GPU-generated chain whose lists have no producer-consumer dependencies on each other. An earlier GPU pass writes the full chain (every list's input and its `NextDispatchList` pointer) into GPU memory, then a single CPU-issued [`DispatchList1`](#dispatchlist1) consumes the whole chain. Because no list reads another's output, the implementation can overlap launches freely; the chain feature avoids what would otherwise be CPU readback of how many lists to issue plus N separate [`DispatchList1`](#dispatchlist1) calls.

**Wait, incrementally generated or sequenced chains.** The app additionally sets [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1), or the list is graphics-class without [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags) (which implies the wait). The implementation defers reading `NextDispatchList` until the current list's work has fully retired. Shaders inside the current list are free to write into the memory holding the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) (and its referenced arrays and record buffers) and then publish the pointer by writing it into the `NextDispatchList` field. The implementation observes the published pointer once the wait completes.
...
Memory visibility of those producer writes (so the next list actually sees them, rather than stale cached values) is the app's call between two options:

- **Shader-managed**: producer shaders use globally-coherent UAV writes. The coherent write is what makes a value visible beyond the local cache; `DeviceMemoryBarrier` orders writes against other operations in the same shader and is not itself what publishes them, so no barrier is required after the final store for the next list to observe it, the wait having retired this list first. Each write pays its own coherence cost; for producer phases with many writes this can be significantly less efficient than the implementation-managed alternative below.
- **Implementation-managed**: the app additionally sets [`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1), and producer shaders can use normal cached writes. The implementation flushes write caches at list-end so the next list reads fresh values. Likely lower overhead when the producer phase has many writes (cache-hit benefit during the writes; one flush at the end instead of per-write coherence).
...
- Compute records (`DISPATCH`) and raytracing records ([`_DISPATCH_RAYS_DIMENSIONS`](#_dispatch_rays_dimensions)) may always be reordered, regardless of the flag above, matching standalone `Dispatch` and `DispatchRays` which have no inter-invocation ordering.
...
13. **[`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1) requires the wait.** A list that sets [`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1) must also have [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) set, either explicitly or implicitly (an in-order graphics-class list (one without [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags)) supplies the implicit wait). Without the wait, "end of list" is not a defined sync point and the flush has no meaningful timing relative to consumers.
...
| Flag | Description |
|---|---|
| `_ALLOW_OUT_OF_ORDER_GRAPHICS` | Permits the implementation to launch and retire graphics-class records (`DRAW`, `DRAW_INDEXED`, `DISPATCH_MESH`) out of `ProgramInputs` order and out of per-program record order. Apps that need rasterization order across the records of a list (e.g. for blending or stencil correctness) must leave this flag clear. Compute-class (`_DISPATCH`, `_FIXED_DISPATCH`) and raytracing-class (`_DISPATCH_RAYS_DIMENSIONS`) records may always be reordered regardless of this flag, see [Execution Order and State Scoping](#execution-order-and-state-scoping). |
...
| `_END_WITH_WAIT_FOR_COMPLETION` | Before considering this list complete, the implementation waits until all work this list launched has fully retired. Within a continuation chain, this defers the read of `NextDispatchList`, and with it the whole of the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) it points at, to the retire point, after any [`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1) this list also requested, required when shaders inside this list write the next list or produce data the next list consumes. Outside a continuation chain it acts as an in-call fence equivalent to a `D3D12_BARRIER` issued immediately after `DispatchList1`. This flag only controls execution sync, not memory visibility, see `_END_WITH_MEMORY_FLUSH` for the cache-flush companion. **Implied for in-order graphics lists**, a graphics-class list without [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags) already requires serial retire to preserve rasterization order, so this flag is redundant in that case. |
| `_END_WITH_MEMORY_FLUSH` | Before considering this list complete, the implementation flushes any pending write caches so that writes performed by this list's shaders become visible to subsequent GPU work, the next list in a continuation chain, or post-call work after `DispatchList1` returns. This lets producer shaders use normal cached UAV writes instead of globally-coherent ones; one flush at list-end covers them all. Must be set together with `_END_WITH_WAIT_FOR_COMPLETION` (explicitly or implied via in-order graphics), without the wait, "end of list" is not a defined sync point and the flush has no meaningful timing relative to consumers. |
...
- **Preemption granularity scope.** Devices report a preemption granularity (the smallest GPU work unit they're willing to preempt at, instruction / dispatch / draw / command-buffer / etc.). How does it compose with [`DispatchList`](#dispatchlist): per record, per list, or only per [`DispatchList`](#dispatchlist) invocation? Matters for apps mixing Work Lists with other workloads on the same queue (priority inversion) and for chains relying on TDR not firing mid-chain.
...
v0.84|8/21/2026|<ul><li>Added introductory diagrams.</li><li>In [Open Issues](#open-issues)</li> listed we need to consider the option for Work Lists to change states like primmitive topology, stencil ref etc., perhaps in a future release to give mroe time, and/or scoped to the most critical to limit complexity.</ul>
