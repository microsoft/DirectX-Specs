...
```
                    ...
                    │  CONTINUATION is set, after the list     │
                    │  retires the GPU reads NextDispatchList  │
                    │  and, if non-null, re-enters as if       │
                    │  DispatchList1 were called again. The    │
                    │  next list's per-call GPU-resident data  │
                    │  (input struct, primary list, secondary  │
                    │  lists) may be authored however the app  │
                    │  wants, CPU-written, written by earlier  │
                    │  GPU work, or written by shaders in the  │
                    │  current list. The SetProgram-bound      │
...
When the implementation reads `NextDispatchList` depends on the current list's ordering, and on whether it sets [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1). That in turn decides whether the next list's launch can overlap the tail of this one:

**Out-of-order list, no wait: pre-staged or asynchronous chains.** A list that sets [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags), or whose class never had a defined order among its records, has no last-retirement to key off, so with only [`_ALLOW_NEXT_DISPATCH_LIST_CONTINUATION`](#d3d12_dispatch_list_flags1) set the app asserts that the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) (the one this list's `NextDispatchList` points at, and what that one references) is valid by the time the implementation reads it, whether the app wrote it from the CPU or had earlier GPU work produce it (with appropriate barriers before the [`DispatchList1`](#dispatchlist1) call). The implementation may read `NextDispatchList` at any time, including at launch, and is free to overlap the launch of the next list with the tail of the current one for tighter scheduling. Subsequent lists deeper in the chain bring their own continuation contract per their own [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) setting, the chain can mix wait and no-wait lists freely. Typical use case: a list earlier in the same chain generates several following lists at once, writing their inputs and their `NextDispatchList` pointers before it finishes. That producer sets the wait, because its consumers depend on its output. The lists it produced do not, since they were all authored before any of them runs, so they can be no-wait and the implementation can overlap their launches. A chain whose contents are all written before the call is a weaker case, since the app could issue the same work as separate [`DispatchList1`](#dispatchlist1) calls.

**In-order list: sequenced by its own last retirement.** A graphics-class list without [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags) retires its records in order, so its last record retiring is a defined point. The implementation reads `NextDispatchList` after that point. This costs nothing beyond the ordering the list already has. Only this list's records are involved, so work that was already running when [`DispatchList1`](#dispatchlist1) was called keeps going. A producer in such a list therefore needs no wait, only a way to publish its writes, which is the choice of two options below. Setting [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) on such a list anyway is legal, and adds only the draining of work issued before the call, such as compute overlapping the list from earlier in the command list. That combination falls out of the flag's definition rather than being designed for, and is unlikely to be what an app wants here.

**Wait: retiring an out-of-order list, or earlier work.** The app sets [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1). The implementation defers reading `NextDispatchList` until preceding work has fully retired. Shaders inside the current list are free to write into the memory holding the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) (and its referenced arrays and record buffers) and then publish the pointer by writing it into the `NextDispatchList` field. The implementation observes the published pointer once the wait completes.
...
Memory visibility of a producer's writes (so the next list actually sees them, rather than stale cached values) is the app's call between two options, whichever route sequenced the two lists:
...
Under either option, a producer publishing to the next list does not need to order its own writes against each other. That relief comes from the sequencing above rather than from the flush: the next list does not begin until the current list has retired, so it cannot observe intermediate state whichever option is chosen. For an in-order list that sequencing is its own last retirement; for one that opted out of ordering it is [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1). What is unnecessary in both cases is splitting the writes into phases: record bodies first, a barrier placed to separate them, then the fields that trigger consumption. What the implementation-managed option changes is the cost of making those writes visible, one flush at list-end rather than per-write coherence. Neither option speaks to ordering that shaders within the current list must observe of each other.
...
13. **The next list is read after an in-order list's last record retires.** A graphics-class list without [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags) retires its records in order. Its last record retiring is therefore a defined point, and `NextDispatchList` is read no earlier. This is sufficient for the chain on its own: the flag is not implied and is not needed. A graphics-class list that sets [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags) has no such point, and neither does any compute-class or raytracing-class list, since those never have a defined order among their records. Such lists get this sequencing only by setting [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1). Sequencing never reaches work that was already running when the [`DispatchList1`](#dispatchlist1) call was made. Only the flag does that.
...
**Within each list, the order described above is strict** (primary list and secondary list validation complete and barrier before record processing begins). **Across continuation boundaries** the implementation has more freedom: when the current list's [`D3D12_DISPATCH_LIST_FLAG1_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) is not set (and the list's class allows out-of-order completion), the GPU may overlap the current list's record processing with reads and primary-list / secondary-list validation dispatches of the next continuation. The next list's record processing still cannot begin until its own signature-selection, primary-list, and secondary-list validation complete and their writes are visible, but the next list's signature-selection and primary-list validation dispatches may begin before the previous list's record processing fully retires. With `_END_WITH_WAIT_FOR_COMPLETION` set, the next continuation's cycle does not begin until the previous list's record processing fully retires.
...
| Flag | Description |
|---|---|
| `_ALLOW_OUT_OF_ORDER_GRAPHICS` | Permits the implementation to launch and retire graphics-class records (`DRAW`, `DRAW_INDEXED`, `DISPATCH_MESH`) out of `ProgramInputs` order and out of per-program record order. Apps that need rasterization order across the records of a list (e.g. for blending or stencil correctness) must leave this flag clear. Compute-class (`_DISPATCH`, `_FIXED_DISPATCH`) and raytracing-class (`_DISPATCH_RAYS_DIMENSIONS`) records are never guaranteed to be launched or retired in `ProgramInputs` order, with or without this flag, see [Execution Order and State Scoping](#execution-order-and-state-scoping). |
...
> **Ordering across a list boundary.** In-order graphics-class lists retire their records in order, so the next list follows the last one. This adds nothing to the ordering the list already has. Only this list's records are involved, so work that was already running when [`DispatchList1`](#dispatchlist1) was called keeps going. A list with no defined order among its records has no last retirement to key off, whether because it set `_ALLOW_OUT_OF_ORDER_GRAPHICS` or because its class never had one. Sequencing the next list after it therefore needs [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1), which retires it. Without it, the later list's records are ordered among themselves but not against the earlier list's, even if the later list is in-order. Implementations may still retire the earlier list's work anyway, and in practice often will, but applications must not depend on it. A list whose shaders produce the next list's input must also make sure its consumer is sequenced after it: an in-order list already is, by its own last retirement, while a list without that point needs the flag (see [Next-list pointer semantics](#next-list-pointer-semantics)).
...
| `_END_WITH_WAIT_FOR_COMPLETION` | Acts like a normal execution barrier in the command list at the end of this list, with the reach that [`D3D12_BARRIER_SYNC_ALL`](D3D12EnhancedBarriers.md#d3d12_barrier_sync_all) describes. What follows, meaning the next list in a continuation chain or the commands recorded after `DispatchList1` in the same command list, does not begin until preceding work has fully retired. That covers everything this list launched and work already submitted on this command list. Within a continuation chain, this defers the read of `NextDispatchList` to the retire point, and with it the whole of the next [`D3D12_DISPATCH_LIST_INPUT1`](#d3d12_dispatch_list_input1) it points at. That happens after any [`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1) this list also requested. A list with no last-retirement point sets it when its shaders write the next list, or produce data the next list consumes. This flag only controls execution sync, not memory visibility, see `_END_WITH_MEMORY_FLUSH` for the cache-flush companion. Never implied: a list that wants the barrier sets it, whatever its class. An in-order graphics-class list does not need it for rasterization order, which retirement order already carries across its records and into the next list, at no cost. The barrier is for retiring a list that has opted out of ordering, and for the data dependency above. A producer with a last-retirement point is already sequenced and needs only to publish its writes. |
| `_END_WITH_MEMORY_FLUSH` | Lets shaders in this list write UAVs normally, without having to make those writes globally coherent, and still have what follows see them. What follows means the next list in a continuation chain, or the commands recorded after `DispatchList1` in the same command list, and the two are treated alike. At the end of this list the implementation makes the writes visible, including any still sitting in caches that later work would not otherwise see. That applies across the whole command list rather than only this list's shaders. [Rasterizer-ordered view](RasterOrderViews.md#semantics) writes are included too, since those are UAV writes as well. Output-merger results (render targets, depth/stencil) are **not** included, and do not need to be. Later lists draw into the same bound targets, blending and depth/stencil testing against earlier output. That is expected to stay coherent across lists as it does across draws, pending the confirmation tracked in [Open Issues](#open-issues). What output-merger results need at a list boundary is *ordering*, from [`_END_WITH_WAIT_FOR_COMPLETION`](#d3d12_dispatch_list_flags1) and [`_ALLOW_OUT_OF_ORDER_GRAPHICS`](#d3d12_dispatch_list_flags), rather than visibility. Independent of `_END_WITH_WAIT_FOR_COMPLETION`: this flag says when this list's writes become visible, the wait says what is ordered after them. An in-order list publishing for the next list needs only this flag, since its consumer is already sequenced after its last retirement. |
...
- **Output-merger coherence across a list boundary.** [`_END_WITH_MEMORY_FLUSH`](#d3d12_dispatch_list_flags1) is scoped to shader writes on the assumption that render-target and depth/stencil output stays coherent across lists in a chain the way it does across consecutive draws, so a later list blending or depth/stencil testing against an earlier list's output needs no end-of-list action. If any implementation needs an explicit end-of-pipe flush between lists for that to hold, we need to know whether it happens silently and at what cost, or whether the spec has to expose it. This is a *visibility* question: whether the results are there to be seen, not whether anything is ordered after them. The same question covers whether retirement order across a boundary is free for in-order lists, which is what lets the wait stay unimplied. TBD with IHVs.
