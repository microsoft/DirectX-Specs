...
v1.44|6/26/2026|<li>In [GetRaytracingAccelerationStructurePrebuildInfo()](#getraytracingaccelerationstructureprebuildinfo) and [D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS](#d3d12_build_raytracing_acceleration_structure_inputs), the spec already said that any parameter that is referenced via `D3D12_GPU_VIRTUAL_ADDRESS` willnot be accessed by the operation. So this memory does not need to be initialized yet or be in a particular resource state. Whether GPU addresses are null or not *can* be inspected by the operation, even though the pointers are not dereferenced.  Added this: If actual GPUVAs aren't known at the time of the call, dummy non-zero values can be used just for the purposes for the prebuild call  forentires that will be replaced with real non-null GPUVAs for build.</li>
