...
- [HLSL](#hlsl)
  ...
  - [HitObject](#hitobject)
    ...
    - [HitObject LoadLocalRootTableConstant](#hitobject-loadlocalroottableconstant)
  - [MaybeReorderThread](#maybereorderthread)
    ...
    - [MaybeReorderThread Example 1](#maybereorderthread-example-1)
    - [MaybeReorderThread Example 2](#maybereorderthread-example-2)
    - [MaybeReorderThread Example 3](#maybereorderthread-example-3)
...
Patching these pointer lists relocates references; it does not replace the referenced objects with different contents. Each non-NULL patched pointer must identify the corresponding original object or a copy of it, including a copy restored by deserialization, with the contents required by the serialized referring object. Those contents must be preserved apart from address relocation and representation changes permitted by the copy or deserialization operation. Independently rebuilding an object with the same build inputs or merely equivalent raytracing behavior is not sufficient to satisfy this requirement.

In particular, a BLAS must be deserialized with references to the corresponding original Opacity Micromap arrays or copies of those arrays, not a different OMM set. Deserialization is not an OMM linkage update, even if the BLAS was built with `ALLOW_OMM_LINKAGE_UPDATE`; changing OMM linkage requires the applicable build or update operation. Likewise, TLAS deserialization must retain references to the corresponding original BLASes or copies of them, not substitute different BLASes. The same relocation-only rule applies to the typed pointer lists for Raytracing2 objects.

This requirement does not impose an order on deserialization. Referenced destination memory may still be uninitialized when the referring object is deserialized, but must contain the required corresponding objects before raytracing or template instantiation uses them. Subsequent changes remain subject to the normal build, update, and reference-validity rules. Violating the pointer-patching requirements above results in undefined behavior.
...
`D3D12_GPU_VIRTUAL_ADDRESS SourceAccelerationStructureData` | <p> Acceleration structure or other type of data to copy/transform based on the specified Mode. The data remains unchanged and usable (such as if it is an acceleration structure). The operation only involves the data pointed to (such as acceleration structure) and not any other data (such as acceleration structures) it may point to. E.g. in the case of a top-level acceleration structure, any bottom-level acceleration structures that it points to are not involved in the operation. Exception: for [Cluster BLAS](raytracing2.md#clustered-geometry) in `_SERIALIZE` or `_VISUALIZATION_DECODE_FOR_TOOLS`, referenced CLAS may be read by the driver and therefore must be valid for the duration of the operation on the GPU timeline.</p><p>Memory must be 256 byte aligned ([D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT](#constants)), or 128 bytes for [Opacity Micromap](#opacity-micromaps) arrays ([D3D12_RAYTRACING_OPACITY_MICROMAP_ARRAY_BYTE_ALIGNMENT](#constants)), regardless of Mode.</p><p>The resource state that the memory pointed to must be in depends on the Mode parameter - see [D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE](#d3d12_raytracing_acceleration_structure_copy_mode) definitions.</p>
...
For Raytracing2 acceleration structure types, see
[D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER](raytracing2.md#d3d12_rtas_tools_visualization_build_inputs_header).
...
`RayDesc Ray` | [Ray](#ray-description-structure) for the miss.
...
`MaybeReorderThread` is available only in shaders of type [raygeneration](#ray-generation-shader).
...
- [Examples](#maybereorderthread-example-1)
- [Reorder Points](#reorder-points)
...
v1.48|9/8/2026|<li>In [D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE](#d3d12_raytracing_acceleration_structure_copy_mode), clarified that tools visualization contains decoded input data but retains external references to built acceleration structures and Opacity Micromap arrays. Distinguished the Cluster BLAS source-liveness requirement from recursively copying referenced CLASes into the visualization result.</li><li>In [D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION_DESC](#d3d12_raytracing_acceleration_structure_postbuild_info_serialization_desc), clarified that deserialization pointer fixups relocate references to corresponding original objects or copies, not substitute different OMM arrays or acceleration structures. Deserialization is not an OMM linkage update. Referenced objects may still be deserialized in any order.</li>
