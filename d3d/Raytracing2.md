...
The formats and operation settings for the returned data are specified by the returned specialized input description; the returned vertex format is always `D3D12_VERTEX_FORMAT_FLOAT32_3` and the returned index format is always `D3D12_INDEX_FORMAT_UINT32`. These fixed formats describe the encoding of the returned vertex and index data only; they do not relax any [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits) maximum. In particular, `MaxVertexCountPerCluster` is limited to a maximum supported value of 256, so emitting one vertex per triangle corner is not valid for a cluster with enough triangles for that to exceed the limit.
