...
[GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo), for retrieving size requirements for an individual operation, shares the [D3D12_RTAS_OPERATION_INPUTS](#d3d12_rtas_operation_inputs) desc with `pInputs` above where the same struct is part of the operation execution definition.
...
The allowance in [Degenerate primitives and instances](raytracing.md#degenerate-primitives-and-instances) to replace the coordinates of discarded degenerate primitives with NaN during visualization applies to vertex positions only; it does not relax the requirements above for geometry indices and triangle flags.

> Note: how the geometry index and flags are defined for a primitive that was discarded from the build, and whether a returned NaN position is guaranteed to reproduce the same discard when a visualization result is used to rebuild, are still being resolved and will be addressed in a future revision.
