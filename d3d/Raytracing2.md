...
For each component in [Compressed1 position encoding](#compressed1-position-encoding), define `I = Anchor + Offset`, where:

- `Anchor` is signed 24-bit two's complement: `[-8,388,608 .. 8,388,607]`
- `Offset` is an unsigned `n`-bit value, `n` in `[1..16]`: `[0 .. (2^n - 1)]`
- `e` is the biased exponent in the `COMPRESSED1` header

`COMPRESSED1` values are valid if:

- `e in [1..232]`
- If `e = 232`, `I in [-8,388,607 .. 8,388,607]`

For `e in [1..231]`, `I` has no additional restriction beyond the encoded range implied by `Anchor` and `Offset`.

Any other case is outside this specification and invokes undefined behavior.
Implementations are not required to clamp or repair invalid values.
...
If this format is `D3D12_VERTEX_FORMAT_COMPRESSED1`, then [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args) has a [D3D12_VERTEX_FORMAT_COMPRESSED1_TEMPLATE_HEADER](#d3d12_vertex_format_compressed1_template_header) `Compressed1.Header` field that specifies a subset of the full `COMPRESSED1` header that is locked for all instances of the template: `exponent`, `x_bits`, `y_bits` and `z_bits`.
...
- `max(I_c) - min(I_c) <= 2^(n_c) - 1` so every offset fits the `n_c`-bit unsigned `Offset_c` field.  Since `n_c` (`x_bits`, `y_bits`, `z_bits`) is locked in the template header, the input position spread per cluster must fit within the locked bit budget.

Validity and undefined behavior for the resulting `COMPRESSED1` values are defined in [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement).
...
For Cluster Geometry types, the returned structures follow similar approximations as bottom-level acceleration structure visualization. Vertex indices might be reshuffled, vertices may be modified in line with acceleration structure tolerances, and geometry indices and triangle flags might be expressed differently but when flattened by the app result in the same value for each triangle as the visualized structure.

The formats and operation settings for the returned data are specified by the returned specialized input description; the returned vertex format is always `D3D12_VERTEX_FORMAT_FLOAT32_3` and the returned index format is always `D3D12_INDEX_FORMAT_UINT32`. These fixed formats describe the encoding of the returned vertex and index data only; they do not relax any [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits) maximum. In particular, `MaxUniqueVertexIndexCountPerCluster` is limited to a maximum supported value of 256, so emitting one vertex per triangle corner is not valid for a cluster with enough triangles for that to exceed the limit.
...
In a visualization result, a discarded primitive must occupy its original position in the returned primitive list, and its returned index values must be degenerate, meaning at least two of the three index values are equal.  Reporting all three index values as `0` satisfies this, referencing whatever vertex is first in the returned vertex data.  [Degenerate primitives and instances](raytracing.md#degenerate-primitives-and-instances) permits such a primitive to be discarded when the visualization result is rebuilt.  The primitive numbering is reproduced whether or not the implementation takes that option: if the rebuild discards the primitive it still consumes its `PrimitiveIndex()` value, and if the rebuild retains it, it occupies that same value and cannot be intersected, since its index values are degenerate.  A discarded primitive may instead be omitted from the returned primitive list if every primitive that follows it in the cluster is also discarded, since omitting a trailing run of discarded primitives cannot change the `PrimitiveIndex()` of any retained primitive.  The returned triangle count may therefore be less than the triangle count the cluster was built from, and is `0` for a cluster in which every primitive is discarded and omitted.  Vertices left unreferenced by omitted primitives need not be returned.

The geometry index, triangle flags and opacity micromap index returned for a discarded primitive may be any values that are legal for the returned build inputs, and are not otherwise constrained, since the primitive cannot be intersected.  Being legal for the returned build inputs includes the geometry index being within `MaxGeometryIndexValue` and counting toward `MaxUniqueGeometryIndexAndFlagsCountPerCluster` as any other primitive's does.  The requirement above that geometry indices and triangle flags flatten to the same value as the visualized structure applies to retained primitives.

No allowance is made for the vertex positions of a discarded primitive.  Positions are per vertex rather than per primitive, and the repeated index values of a discarded primitive may reference a vertex that retained primitives also reference, so altering them could move a retained triangle.  All vertex positions returned for cluster geometry are finite, so the allowance in [Degenerate primitives and instances](raytracing.md#degenerate-primitives-and-instances) to replace the coordinates of discarded degenerate primitives with NaN during visualization does not apply to cluster geometry.

A CLAS is described with `D3D12_RTAS_OPERATION_TYPE_BUILD_CLAS_FROM_TRIANGLES` regardless of whether the visualized CLAS was originally built from triangles directly or instantiated from a Cluster Template. For templates, hint vertices need not be returned unless required by the returned build inputs.
...
- Conversion to [COMPRESSED1](#compressed1-position-encoding) format: Round to nearest, ties toward `+infinity` (e.g. `floor(x + 0.5f)`).
  - This non-IEEE rounding choice is intentional: it is simple, sign-symmetric, and doesn't switch behavior across the origin (so a fixed-point spatial encoding stays consistent regardless of where the cluster sits in world space).
  - Quantization is defined by `Vq = round(V / s) * s`, where `s = 2^(e-127)` and `e` is the selected biased exponent.
  - Equivalent integer form: `I = round(V / s)`, then `Vq = I * s`.
  - At template instantiation, the anchor is derived per component as `min(I)` over the already-quantized integer values across all input positions.  Because the anchor is selected from the quantized set rather than rounded independently, no separate rounding mode applies to it, and the vertex that determined the anchor naturally encodes to an offset of exactly `0`.
  - Per-vertex offsets are encoded from the quantized integer values relative to that anchor.
  - Validity and undefined behavior for resulting `COMPRESSED1` values are defined in [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement).
...
A primitive that is discarded during a cluster build still consumes its `PrimitiveIndex()` value.  The conditions under which a primitive may be discarded are those in [Degenerate primitives and instances](raytracing.md#degenerate-primitives-and-instances).  This applies equally when a CLAS is instantiated from a cluster template, which is where positions first arrive and where conversion to `VertexInstantiationFormat` can make a primitive degenerate, so instantiation must not renumber either.  The primitive indices of the retained primitives in a cluster are therefore unaffected by any discard, so an application can index its own data by `PrimitiveIndex()` without regard to what the implementation discarded.  How discarded primitives appear in a tools visualization result is described in [D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER](#d3d12_rtas_tools_visualization_build_inputs_header).
