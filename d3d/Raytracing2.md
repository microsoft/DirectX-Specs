...
- [Clustered geometry](#clustered-geometry)
  ...
  - [Cluster input geometry encoding](#cluster-input-geometry-encoding)
    - [Triangle participation in cluster operations](#triangle-participation-in-cluster-operations)
    - [Compressed1 position encoding](#compressed1-position-encoding)
      - [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement)
  ...
  - [Command list methods](#command-list-methods)
    - [ExecuteIndirectRTASOperations](#executeindirectrtasoperations)
      - [ExecuteIndirectRTASOperations Structures](#executeindirectrtasoperations-structures)
        ...
        - [D3D12\_RTAS\_TOOLS\_VISUALIZATION\_BUILD\_INPUTS\_HEADER](#d3d12_rtas_tools_visualization_build_inputs_header)
...
- Calls to [CopyRaytracingAccelerationStructure](Raytracing.md#copyraytracingaccelerationstructure) and [EmitRaytracingAccelerationStructurePostbuildInfo](Raytracing.md#emitraytracingaccelerationstructurepostbuildinfo) are not supported, **except** for the serialize/deserialize and tools-visualization modes documented in the base spec: specifically `D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE`, [`_DESERIALIZE`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), and [`_VISUALIZATION_DECODE_FOR_TOOLS`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), along with the corresponding [`D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION`](raytracing.md#d3d12_raytracing_acceleration_structure_postbuild_info_type) and `_TOOLS_VISUALIZATION` postbuild-info types used to size their outputs.  Compaction and clone copy modes are not supported; `D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS` should be used to copy/move these objects. For Cluster BLAS in `_SERIALIZE` or `_VISUALIZATION_DECODE_FOR_TOOLS`, and for corresponding postbuild sizing queries, all referenced CLAS must be valid for the duration of the operation on the GPU timeline. This does not constrain deserialization order semantics, which remain as defined by the base serialization/deserialization behavior.
...
Accessing acceleration structures that may contain clusters from shaders requires opting in via [D3D12_RAYTRACING_PIPELINE_FLAG_ALLOW_CLUSTERED_GEOMETRY](raytracing.md#d3d12_raytracing_pipeline_flags) flag for the state object, or for [RayQuery](#rayquery), [RAYQUERY_FLAG_ALLOW_CLUSTERED_GEOMETRY](raytracing.md#rayquery-flags) in the `RayQuery` instantiation.  Shaders compiled before Shader Model 6.9 cannot specify [RAYQUERY_FLAG_ALLOW_CLUSTERED_GEOMETRY](raytracing.md#rayquery-flags), since the template parameter carrying it was added in Shader Model 6.9; those shaders opt in at pipeline creation instead, see [RayQuery flags on pre Shader Model 6.9 shaders](raytracing.md#rayquery-flags-on-pre-shader-model-69-shaders).
...
- CLAS can only contain a limited number of triangles and unique vertex indices. The limit is 256 triangles and 256 unique vertex indices, one cluster per CLAS. Vertex identity for this limit is the numeric index value, not equality of the position data stored in the referenced vertex-buffer elements.
...
Cluster BLAS objects are not compatible with [BuildRaytracingAccelerationStructure()](raytracing.md#buildraytracingaccelerationstructure).  They are also not compatible with [CopyRaytracingAccelerationStructure()](raytracing.md#copyraytracingaccelerationstructure) and [EmitRaytracingAccelerationStructurePostbuildInfo()](raytracing.md#emitraytracingaccelerationstructurepostbuildinfo), **except** for the serialize/deserialize and tools-visualization modes: `D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE`, [`_DESERIALIZE`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), and [`_VISUALIZATION_DECODE_FOR_TOOLS`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), plus the corresponding [`POSTBUILD_INFO_SERIALIZATION`](raytracing.md#d3d12_raytracing_acceleration_structure_postbuild_info_type) and `_TOOLS_VISUALIZATION` postbuild-info types used to size their outputs. For Cluster BLAS in `_SERIALIZE` or `_VISUALIZATION_DECODE_FOR_TOOLS`, and for corresponding postbuild sizing queries, all referenced CLAS must be valid for the duration of the operation on the GPU timeline. This does not constrain deserialization order semantics, which remain as defined by the base serialization/deserialization behavior.  

Instead, all cluster-level acceleration structure functionality is provided through GPU-driven [indirect acceleration structure operations](#indirect-acceleration-structure-operations). In particular [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) must be used to perform various tasks such as builds, fetching sizes and copying.
...
### Triangle participation in cluster operations

A cluster triangle is participating unless this section classifies it as non-participating.  Participation is independent of any ray; a non-participating triangle produces no intersections.  The [inactive](raytracing.md#inactive-primitives-and-instances) and [degenerate](raytracing.md#degenerate-primitives-and-instances) classifications for non-cluster BLAS do not apply to cluster triangles.  These rules do not constrain internal representation.

A triangle with fewer than three distinct position indices is non-participating, including in any CLAS instantiated from a Cluster Template.  A position is *relevant* if referenced by a triangle with three distinct indices.  Position-value requirements apply only to relevant positions, and relevance depends only on index data.

Non-finite means NaN or infinity.  Positions compare equal when all three components are numerically equal.

For a direct CLAS build, positions are interpreted according to `VertexFormat` and expressed as float32 before applying `PositionTruncateBitCount`.  A triangle with three distinct indices is non-participating if any x component is non-finite.  Otherwise, every component must be finite or behavior is undefined; after truncation, the triangle is non-participating if any two positions compare equal.

For an optional Cluster Template hint `VertexBuffer`, a triangle with three distinct indices provides no position hint if any x component is non-finite; otherwise its components must be finite or behavior is undefined.  Hints do not affect template topology or functional behavior.

During Cluster Template instantiation, every relevant source position must be finite after interpretation according to `VertexSourceFormat` and before conversion to `VertexInstantiationFormat`; otherwise behavior is undefined.  After conversion and `PositionTruncateBitCount`, a triangle with three distinct indices is non-participating if any two positions compare equal.

---

### Compressed1 position encoding

[D3D12_VERTEX_FORMAT_COMPRESSED1](#d3d12_vertex_format) vertices use this layout without padding between sections and are referenced by GPU-side [cluster build arguments](#d3d12_rtas_operation_build_clas_from_triangles_args) and [Cluster Template instantiation arguments](#d3d12_rtas_operation_instantiate_cluster_templates_args):
...
- Power of 2 exponent value encoding a value in range `[-126..105]` (biased range `[1..232]`)
  - Validity constraints and undefined behavior are defined in [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement).
- 24bit * 3 (xyz) anchor position encoding
- Bit sizes for per-vertex xyz deltas from anchor (specifying `1-16` bits per component) 

> Note vertex count isn't needed here as it is part of the overall cluster description.  See `UniqueVertexIndexCount` in GPU-side build arguments: [D3D12_RTAS_OPERATION_BUILD_CLAS_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_clas_from_triangles_args) and [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args). `UniqueVertexIndexCount` counts distinct referenced index values; it does not define the encoded vertex block's extent. The block must contain a complete encoded element for every index referenced by the cluster. Also see the `Max*` counts in CPU-side build inputs description: [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits).
...
The biased exponent `e` in a full or Cluster Template `COMPRESSED1` header must be in `[1..232]`.  For each component of every [relevant position](#triangle-participation-in-cluster-operations), define `I = Anchor + Offset`, where:

- `Anchor` is signed 24-bit two's complement: `[-8,388,608 .. 8,388,607]`
- `Offset` is an unsigned `n`-bit value, `n` in `[1..16]`: `[0 .. (2^n - 1)]`

If `e = 232`, `I` must be in `[-8,388,607 .. 8,388,607]`.  For `e` in `[1..231]`, `I` has no additional restriction beyond the encoded range implied by `Anchor` and `Offset`.

Violating these requirements is undefined behavior; implementations need not clamp or repair invalid values.
...
`TemplateInstancePrefersCOMPRESSED1` in [D3D12_FEATURE_D3D12_OPTIONS_NNN](raytracing.md#d3d12_feature_d3d12_options_nnn) reports the device preference. If `TRUE`, applications that are willing to make a per-device template format selection should prefer `D3D12_VERTEX_FORMAT_COMPRESSED1`. If `FALSE`, such applications should prefer another format, such as `D3D12_VERTEX_FORMAT_FLOAT32_3`, optionally using `PositionTruncateBitCount` to select the stored precision. This is a performance preference only: every valid `VertexInstantiationFormat` is supported regardless of device. It does not apply to static CLAS builds.

This preference applies only to `VertexInstantiationFormat`, which defines the functional precision and range stored in the instantiated CLAS. `VertexSourceFormat` is selected independently and may be any format valid for template instantiation; the device converts the input positions to `VertexInstantiationFormat` during instantiation. See [Template instance format conversion semantics](#template-instance-format-conversion-semantics).

Independently of this preference, an application can optionally provide a useful [`InstantiationBoundingBoxLimit`](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args) for any selected template format. This gives the implementation advance bounds on future instance positions and may reduce conservative storage or template-instantiation work.

If `VertexInstantiationFormat` is `D3D12_VERTEX_FORMAT_COMPRESSED1`, [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args) provides `Compressed1.Header`, which sets the instance `exponent`, `x_bits`, `y_bits`, and `z_bits`.

A `COMPRESSED1` source is source-compatible when each source bit count is no greater than its template counterpart.  Source-compatible conversion uses the source anchor, exponent, and offsets, zero-extending offsets to the template bit widths; the template exponent is ignored.

Otherwise, using the template exponent and quantized integer `I` from [Template instance format conversion semantics](#template-instance-format-conversion-semantics), each anchor component is `min(I)` over relevant positions and each relevant-position offset is `I - Anchor`.  If there are no relevant positions, the anchor is unconstrained.

On this path, if at least one relevant position exists, behavior is undefined unless, for each component `c`:

- `min(I_c)` in `[-8,388,608 .. +8,388,607]` so the anchor fits the 24-bit signed `Anchor_c` field.
- `max(I_c) - min(I_c) <= 2^(n_c) - 1` so every relevant offset fits the locked `n_c`-bit unsigned `Offset_c` field.

All resulting `COMPRESSED1` values must also satisfy [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement).

On the source-compatible path, watertightness across clusters, when required, depends on the source anchors and exponents.
...
Instead, just like with [CLAS](#cluster-level-acceleration-structure), all PTLAS related functionality is provided through GPU-driven [indirect acceleration structure operations](#indirect-acceleration-structure-operations). [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) must be used to perform various tasks such as builds and fetching sizes.
...
Member                           | Definition
------                           | ----------
`D3D12_EXECUTE_INDIRECT_RTAS_OPERATIONS_FLAG_NONE` | No flags are defined yet - for expansion.
...
Member                                    | Definition
------                                    | ----------
`D3D12_RTAS_OPERATION_INPUTS Inputs` | Input parameters for the operation. See [D3D12_RTAS_OPERATION_INPUTS](#d3d12_rtas_operation_inputs). The `Inputs.Type` member ([D3D12_RTAS_OPERATION_TYPE](#d3d12_rtas_operation_type)) dictates which union member below is used.
...
[ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) points to an array of [D3D12_RTAS_OPERATION_DESC](#d3d12_rtas_operation_desc) structs defining independent sets of RTAS operations to perform.  The operation types are listed below along with related fields for the given type (field [Inputs](#d3d12_rtas_operation_desc)`.Type`).

[GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo), for retrieving size requirements for an individual operation, shares the [D3D12_RTAS_OPERATION_INPUTS](#d3d12_rtas_operation_inputs) desc with `Inputs` above where the same struct is part of the operation execution definition.
...
When [D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_VISUALIZATION_DECODE_FOR_TOOLS](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode) is used on [CLAS, Cluster Templates, Cluster BLAS](#clustered-geometry), or [Partitioned TLAS](#partitioned-top-level-acceleration-structures), the data following [D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_TOOLS_VISUALIZATION_HEADER](raytracing.md#d3d12_build_raytracing_acceleration_structure_tools_visualization_header) begins with `D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER`.

This header is followed by `BuildInputsSizeInBytes` bytes containing the specialized input description selected by `OperationType`, followed by the visualization payload.  `BuildInputsSizeInBytes` does not include any padding after the specialized input description.  The visualization payload begins at the next offset after the specialized input description that satisfies the natural alignment of the payload element type listed in the table below.  Padding bytes, if any, are undefined and must be ignored by readers.  The visualization payload contains the GPU-side arguments or operation list for the selected operation type.  GPU virtual addresses that point to decoded input data, such as vertex buffers, index buffers, and arrays of object addresses, point within the visualization result buffer. GPU virtual addresses that reference already-built acceleration structures or Opacity Micromap arrays instead retain references to those external objects; the objects are not copied into the visualization result buffer.

For Cluster BLAS visualization, `ClasAddressArray` points to an array inside the result buffer, but each entry in that array references an existing built CLAS. For CLAS or Cluster Template visualization, the returned OMM index data is inside the result buffer, while a non-NULL `OpacityMicromapArray` references the existing built Opacity Micromap array. Likewise, the `AccelerationStructure` field of a returned Partitioned TLAS instance references an existing built BLAS. These object references are not pointers to the objects' separately decoded visualization data. Tools that need to inspect a referenced object's contents decode that object separately.

When the returned data is used with [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) for its `OperationType`, rebuilding must preserve trace intersections and shader-visible behavior within the tolerances and normalizations permitted for tools visualization, except that exact duplicate counts for any-hit shader invocations and non-opaque `RayQuery` candidates may differ; duplicate-free behavior must be preserved.  For a Cluster Template, this equivalence applies to corresponding CLAS instantiated with the same `VertexSourceFormat`, vertex values, `GeometryIndexOffset`, and `ClusterIdOffset`.  The returned data need not preserve the original build call, batching, memory placement, performance, operation mode, or source encodings.

Member                               | Definition
------                               | ----------
`D3D12_RTAS_OPERATION_TYPE OperationType` | Operation type that identifies the specialized input description following this header and the kind of GPU-side payload following that description.  Must be one of the operation types listed in the table below.
`UINT BuildInputsSizeInBytes`        | Size in bytes of the specialized input description immediately following this header.  This is the size of the input description struct selected by `OperationType`.

`OperationType`                                           | Specialized input description following this header | Visualization payload following the input description
---------------                                           | --------------------------------------------------- | -----------------------------------------------------
`D3D12_RTAS_OPERATION_TYPE_BUILD_CLAS_FROM_TRIANGLES`      | [D3D12_RTAS_CLUSTER_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_triangles_inputs_desc) | [D3D12_RTAS_OPERATION_BUILD_CLAS_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_clas_from_triangles_args), with `NumDescs == 1`
`D3D12_RTAS_OPERATION_TYPE_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES` | [D3D12_RTAS_CLUSTER_TEMPLATE_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_template_triangles_inputs_desc) | [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args), with `NumDescs == 1`
`D3D12_RTAS_OPERATION_TYPE_BUILD_BLAS_FROM_CLAS`           | [D3D12_RTAS_CLAS_INPUTS_DESC](#d3d12_rtas_clas_inputs_desc) | [D3D12_RTAS_OPERATION_BUILD_BLAS_FROM_CLAS_ARGS](#d3d12_rtas_operation_build_blas_from_clas_args), with `NumDescs == 1`
`D3D12_RTAS_OPERATION_TYPE_PARTITIONED_TLAS`               | [D3D12_RTAS_PARTITIONED_TLAS_INPUTS_DESC](#d3d12_rtas_partitioned_tlas_inputs_desc) | [D3D12_RTAS_PARTITIONED_TLAS_OPERATION](#d3d12_rtas_partitioned_tlas_operation) array, with `NumDescs` elements
...
Subject to this equivalence, returned CLAS data may replace triangles using repeated indices or equal positions, and returned Cluster Template data may replace triangles using repeated indices.  Replacements remain at their original `PrimitiveIndex()`; a trailing run may instead be omitted.

Returned CLAS vertex indices may be remapped when packing the returned vertex data, and positions may be modified within acceleration structure tolerances.  Retained triangles in Cluster Template data preserve their numeric index values because the corresponding vertex positions are supplied when the template is instantiated.

The specialized input description defines the returned formats and operation settings.  For CLAS data, the returned `VertexFormat` is `D3D12_VERTEX_FORMAT_FLOAT32_3` and the returned `IndexFormat` is `D3D12_INDEX_FORMAT_UINT8`; implementations must pack or remap the returned vertices and indices as needed so every index is representable in `UINT8`.  For Cluster Template data, the returned `IndexFormat` is `D3D12_INDEX_FORMAT_UINT32` and hint vertices need not be returned.  The fixed CLAS vertex format does not apply to `VertexInstantiationFormat`.

For returned specialized input descriptions that contain [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits), the `ClusterLimits` member must contain values sufficient for the returned visualization payload. These values are not required to match the values used by the application when the visualized object was originally built.

Per-triangle geometry index, triangle flag, and opacity micromap values for replacement triangles need only be legal for the returned build inputs.  All returned vertex positions must be finite.

A CLAS is described with `D3D12_RTAS_OPERATION_TYPE_BUILD_CLAS_FROM_TRIANGLES` regardless of whether it was built directly or instantiated from a Cluster Template.

For a Partitioned TLAS, the returned [D3D12_RTAS_PARTITIONED_TLAS_OPERATION](#d3d12_rtas_partitioned_tlas_operation) array describes the current content of the structure and is valid to apply to an empty Partitioned TLAS created according to the returned [D3D12_RTAS_PARTITIONED_TLAS_INPUTS_DESC](#d3d12_rtas_partitioned_tlas_inputs_desc).

Pointer fields for caller-owned destinations, scratch storage, result-address arrays, result-size arrays, and tools-info storage are supplied by the caller according to the normal [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) rules.

Used by:
- [D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_TOOLS_VISUALIZATION_HEADER](raytracing.md#d3d12_build_raytracing_acceleration_structure_tools_visualization_header) for Raytracing2 acceleration structure types
...
typedef struct D3D12_RTAS_CLUSTER_LIMITS
{
    ...
    UINT MaxUniqueVertexIndexCountPerCluster;
    UINT MaxTotalTriangleCount;
    UINT MaxSumOfUniqueVertexIndexCounts;
...
`UINT MaxUniqueVertexIndexCountPerCluster`        | The maximum `UniqueVertexIndexCount` of any individual CLAS or Cluster Template, maximum supported value: 256. Vertex identity is the numeric index value, not equality of position data in the referenced vertex-buffer elements.  Scope is a given build operation.
...
`UINT MaxSumOfUniqueVertexIndexCounts`             | The maximum allowed sum of `UniqueVertexIndexCount` over all argument structures passed to the build operation. Each CLAS or Cluster Template contributes its own count; the same numeric index value or equal position data in different clusters is counted once per cluster. If the sum exceeds this value, behavior is undefined.  Scope is a given build operation.
...
The [GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo) call will return sufficient memory requirements for a corresponding operation in [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) if the limits in `ClusterLimits` are respected, the fields with names beginning with `Max` are greater than or equal to the actual maximums and all the fields with names beginning with `Min` are less than or equal to the actual minimum. All other fields must match the exact values used.
...
`D3D12_VERTEX_FORMAT VertexInstantiationFormat`           | The vertex format that will be used by the vertex positions in the eventually instantiated CLAS. Applications should select this based on [`TemplateInstancePrefersCOMPRESSED1`](raytracing.md#templateinstancepreferscompressed1). See [D3D12_VERTEX_FORMAT](#d3d12_vertex_format), [Compressed1 with cluster templates](#compressed1-with-cluster-templates), and [Template instance format conversion semantics](#template-instance-format-conversion-semantics).
`D3D12_INDEX_FORMAT IndexFormat`           | Position index format.  See [D3D12_INDEX_FORMAT](#d3d12_vertex_format).  `D3D12_INDEX_FORMAT_NONE` is not allowed for positions - there must be an index buffer for cluster build inputs.
`D3D12_INDEX_FORMAT GeometryIndexAndFlagsIndexFormat`   | A value of [D3D12_INDEX_FORMAT](#d3d12_index_format) describing the index format referenced by the `GeometryIndexAndFlagsIndexBuffer` field in build args. `D3D12_INDEX_FORMAT_NONE` means sequential indexing, ignoring the index buffer field. If the index buffer field is null that also applies sequential indexing, causing `IndexFormat` to be ignored.  For general geometry index discussion see [The role of geometry index in a cluster](#the-role-of-geometry-index-in-a-cluster).
`D3D12_INDEX_FORMAT OpacityMicromapIndexFormat`         | A value of [D3D12_INDEX_FORMAT](#d3d12_index_format) describing the index format referenced by the `OpacityMicromapIndexBuffer` field in build args. `D3D12_INDEX_FORMAT_NONE` means sequential indexing, ignoring the index buffer field. If the index buffer field is null that also applies sequential indexing, causing `IndexFormat` to be ignored.
...
At instantiation, the device converts input positions to `VertexInstantiationFormat`.  If every component of a source position is finite, conversion to any uncompressed `VertexInstantiationFormat` produces a result in which every component is finite.  The rounding that is applied to fit the precision of the destination format is as follows:
...
- Conversion to [COMPRESSED1](#compressed1-position-encoding) format: Round to nearest, ties toward `+infinity` (e.g. `floor(x + 0.5f)`).
  - With `s = 2^(e-127)`, quantization is `I = round(V / s)` and `Vq = I * s`.
  - Anchor and offset generation, including source-compatible conversion, is defined in [Compressed1 with cluster templates](#compressed1-with-cluster-templates).
  - Validity is defined in [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement).
...
Whether a CLAS is built directly or instantiated from a Cluster Template, a non-participating triangle still consumes its `PrimitiveIndex()` value, so participating triangles retain their indices.  Applications may therefore index their data by `PrimitiveIndex()` without depending on the internal representation of non-participating triangles.  [D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER](#d3d12_rtas_tools_visualization_build_inputs_header) defines their tools-visualization representation.
...
`D3D12_RTAS_OPERATION_MODE_GET_SIZES`            | <p>Specify that the operation must not write any results but only populate the array referenced by `ResultSizeArray` in [D3D12_RTAS_BATCHED_OPERATION_DATA](#d3d12_rtas_batched_operation_data) with the sizes of the results. For operation types other than [D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS](#d3d12_rtas_operation_type), this mode is for per-result sizing with explicit destinations; sizing for implicit destinations must use [GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo) — see [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info) for the formal `_GET_SIZES`/prebuild inequalities and the implicit-layout freedom that makes summed `_GET_SIZES` insufficient for implicit sizing. If used with [D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS](#d3d12_rtas_operation_type), fetches the sizes of existing Cluster Templates, Cluster BLAS or CLAS.</p><p>For cluster builds and template instantiation, providing the `VertexBuffer` in the applicable `_ARGS` structure allows `GET_SIZES` to return a tighter size. If `VertexBuffer` is `null`, the returned size is conservative and no larger than the result from [GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo). Some devices and formats, such as [COMPRESSED1](#compressed1-position-encoding), may return the same template-instantiation size with or without a `VertexBuffer`.</p><p>For a CLAS or Cluster Template build with a null `VertexBuffer`, `IndexBuffer` may also be null, producing a conservative size. If `IndexBuffer` is provided without `VertexBuffer`, the size remains valid when the actual build omits any subset of the queried triangles.</p><p>Contents of other buffers referenced by `_ARGS`, including OMM and geometry index/flags buffers and their index buffers, must match the actual build for triangles retained from the query. Values referenced only by omitted triangles need not match.</p>

For cluster template builds, see [`InstantiationBoundingBoxLimit`](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args) for its `GET_SIZES` access and subsequent-build matching requirements.
...
See also [Why compaction doesn't apply to clustered geometry](#why-compaction-doesnt-apply-to-clustered-geometry).
...
The [GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo) call will return sufficient memory requirements for a call to [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) if all fields with names beginning with `Max` are greater than or equal to the actual maximums. All other fields must match the exact values used.
...
`UINT MaxArgCount`                                               | Maximum number of argument structures that will be provided to the indirect operation. When the operation is executed on the GPU timeline, one operation will be executed for each argument provided. To control the actual number of arguments, see `IndirectArgumentArraySize` in [D3D12_RTAS_BATCHED_OPERATION_DATA](#d3d12_rtas_batched_operation_data).
`D3D12_RTAS_OPERATION_MODE Mode`    | Operation mode (implicit/explicit destinations, get sizes). See [D3D12_RTAS_OPERATION_MODE](#d3d12_rtas_operation_mode).
`UINT MaxTotalClasCount`                                         | 	The maximum allowed value for the sum of the values of `ClasAddressCount` in [D3D12_RTAS_OPERATION_BUILD_BLAS_FROM_CLAS_ARGS](#d3d12_rtas_operation_build_blas_from_clas_args) over all argument structures passed to the operation. If the sum of the values of `ClasAddressCount` exceeds this value during a call to [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) the behavior is undefined.
`UINT MaxClasCountPerArg`                                        | The maximum allowed value of any individual `ClasAddressCount` in [D3D12_RTAS_OPERATION_BUILD_BLAS_FROM_CLAS_ARGS](#d3d12_rtas_operation_build_blas_from_clas_args) in any of the argument structures passed to the operation. If exceeded, behavior is undefined.
...
`UINT16 UniqueVertexIndexCount`                | Exact number of distinct numeric index values in `IndexBuffer` when both `TriangleCount` and `UniqueVertexIndexCount` are nonzero. Vertex identity is the index value, not equality of the position data in the referenced vertex-buffer elements, so different indices containing equal positions count separately. Index values need not be dense and may exceed `UniqueVertexIndexCount - 1`; `UniqueVertexIndexCount` is neither the extent of `VertexBuffer` nor one greater than the maximum index. Every referenced index must address valid vertex data according to `VertexFormat`, including a complete encoded element in a [COMPRESSED1](#compressed1-position-encoding) vertex block.  Valid range: `[0..256]`. If either `TriangleCount` or `UniqueVertexIndexCount` is `0`, the result is an empty `CLAS` that can still be used, but will obviously never produce any hits; the two counts may be zero independently.  The same definition applies when constructing a Cluster Template: the count comes from the template's index buffer, even though the vertex data itself is supplied at instantiation.  `VertexBuffer` is the only field whose optionality is added by the template case, see [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args).
...
`D3D12_GPU_VIRTUAL_ADDRESS VertexBuffer`  | <p>The address where vertex data starts.  Contents depend on selected [D3D12_VERTEX_FORMAT](#d3d12_vertex_format).  Address must align vertex data to component size.  For compressed formats such as [COMPRESSED1](#compressed1-position-encoding), the format defines the required alignment.  Position-value requirements and their effect on tracing are defined in [Triangle participation in cluster operations](#triangle-participation-in-cluster-operations).</p><p>For [Cluster Template construction](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args), `VertexBuffer` may be null or supplied as a hint. See [Cluster templates](#cluster-templates) and [resource state for read-only arguments](#resource-state-for-read-only-arguments).</p>
...
`D3D12_GPU_VIRTUAL_ADDRESS OpacityMicromapArray` | Either NULL or the address of a valid Opacity Micromap Array to reference from the CLAS. If NULL, and the OMM index buffer is effectively absent (either `OpacityMicromapIndexBuffer` is NULL, or `OpacityMicromapIndexFormat` is `D3D12_INDEX_FORMAT_NONE`), [Opacity Micromaps](raytracing.md#opacity-micromaps) will be disabled for this CLAS. If `OpacityMicromapArray` is non-NULL, or the OMM index buffer is effectively present (`OpacityMicromapIndexBuffer` non-NULL and `OpacityMicromapIndexFormat` not `D3D12_INDEX_FORMAT_NONE`), [D3D12_RTAS_OPERATION_FLAG_ALLOW_OMM](#d3d12_rtas_operation_flags) must be set with this call and all calls to [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) interacting with the resulting object of this call, otherwise behavior is undefined.  Address must be aligned to [D3D12_RAYTRACING_OPACITY_MICROMAP_ARRAY_BYTE_ALIGNMENT](raytracing.md#constants) (`128`). See [resource state for read-only arguments](#resource-state-for-read-only-arguments).
...
GPU-side arguments for building a cluster template from triangle data. `VertexInstantiationFormat` in [D3D12_RTAS_CLUSTER_TEMPLATE_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_template_triangles_inputs_desc) selects the format-specific union arm for every element in the operation's `IndirectArgumentArray`.

Member                                    | Definition
------                                    | ----------
`D3D12_RTAS_OPERATION_BUILD_CLAS_FROM_TRIANGLES_ARGS TrianglesArgs`                          | Most arguments match [D3D12_RTAS_OPERATION_BUILD_CLAS_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_clas_from_triangles_args).  For template construction, `VertexBuffer` is optional and `VertexHintFormat` determines its format.  See [Cluster templates](#cluster-templates) and [Triangle participation in cluster operations](#triangle-participation-in-cluster-operations).
`D3D12_GPU_VIRTUAL_ADDRESS InstantiationBoundingBoxLimit` | <p>Either NULL or the address of a bounding box. The bounding box, if provided, is defined by six 32-bit floating point values ordered `MinX`, `MinY`, `MinZ`, `MaxX`, `MaxY`, `MaxZ` and aligned to 32 bytes, see [`D3D12_RAYTRACING_CLUSTER_TEMPLATE_BOUNDS_BYTE_ALIGNMENT`](#constants). Every value must be finite, and `MinX <= MaxX`, `MinY <= MaxY`, and `MinZ <= MaxZ`. If any of these requirements is not met, behavior is undefined.</p><p>If non-NULL, the bounding box applies to each [relevant](#triangle-participation-in-cluster-operations) input position interpreted according to `VertexSourceFormat` as float32. Every such position supplied to an instantiation must lie within the box; otherwise behavior is undefined. Positions after conversion to `VertexInstantiationFormat` or application of `PositionTruncateBitCount` may lie outside the box. The memory is not accessed after the template is built.</p><p>If NULL, no additional restrictions apply. This member applies to every `VertexInstantiationFormat`, including [D3D12_VERTEX_FORMAT_COMPRESSED1](#d3d12_vertex_format).</p><p>In `D3D12_RTAS_OPERATION_MODE_GET_SIZES`, a non-NULL `InstantiationBoundingBoxLimit` is read and used for sizing just as it is for a template build. To use that size for a subsequent `D3D12_RTAS_OPERATION_MODE_EXPLICIT_DESTINATIONS` build, the subsequent build must use the same nullness and, when non-NULL, the same six bounding-box values. See [resource state for read-only arguments](#resource-state-for-read-only-arguments).</p>
`D3D12_VERTEX_FORMAT_COMPRESSED1_TEMPLATE_HEADER Compressed1.Header` | When `VertexInstantiationFormat` in [D3D12_RTAS_CLUSTER_TEMPLATE_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_template_triangles_inputs_desc) is [D3D12_VERTEX_FORMAT_COMPRESSED1](#d3d12_vertex_format), `Compressed1.Header` is used. See [Compressed1 with cluster templates](#compressed1-with-cluster-templates) and [Template instance format conversion semantics](#template-instance-format-conversion-semantics). See [resource state for read-only arguments](#resource-state-for-read-only-arguments).
`UINT Compressed1.ReservedPadding` | Reserved padding in the `Compressed1` format-specific arguments. Must be set to `0` when `VertexInstantiationFormat` is `D3D12_VERTEX_FORMAT_COMPRESSED1`.
...
HLSL does not support unions, so applications author a flattened variant matching the `VertexInstantiationFormat` selected in [D3D12_RTAS_CLUSTER_TEMPLATE_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_template_triangles_inputs_desc). Examples follow, assuming the nested structs have also been declared in HLSL:
...
Referenced by `Compressed1.Header` member of [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args).
...
v0.25|5/1/2026|<li>Added [COMPRESSED1 finite float32 requirement](#compressed1-finite-float32-requirement) section formalizing exponent and integer-position validity for the `COMPRESSED1` vertex format (biased exponent `e in [1..232]`; if `e = 232`, `I = Anchor + Offset in [-8,388,607 .. +8,388,607]`).  Removed the prior "implementations must clamp to this valid range" obligation in [Compressed1 position encoding](#compressed1-position-encoding) - out-of-range encodings are now undefined behavior and implementations are not required to clamp or repair them.  Cross-referenced the new validity rules from [Compressed1 with cluster templates](#compressed1-with-cluster-templates), [Conversion to COMPRESSED1 under Template instance format conversion semantics](#template-instance-format-conversion-semantics), and the `VertexBuffer` field of [D3D12_RTAS_OPERATION_INSTANTIATE_CLUSTER_TEMPLATES_ARGS](#d3d12_rtas_operation_instantiate_cluster_templates_args).  In [Compressed1 with cluster templates](#compressed1-with-cluster-templates), formalized the `s = 2^(e-127)` / `I = round(V / s)` quantization with anchor = `min(I)` per component, and added the per-cluster bit-budget constraint (`max(I_c) - min(I_c) <= 2^(n_c) - 1`) that input positions must respect for the encoding to be valid.  In [Conversion to COMPRESSED1 under Template instance format conversion semantics](#template-instance-format-conversion-semantics), corrected the rounding example: the formula for "round to nearest, ties toward `+infinity`" is `floor(x + 0.5f)` (the prior `trunc(x + 0.5f)` example rounds negative non-tie values toward zero rather than to the nearest integer).  Added the `Stored biased range: [1..232]` annotation to the [D3D12_VERTEX_FORMAT_COMPRESSED1_TEMPLATE_HEADER](#d3d12_vertex_format_compressed1_template_header) bit-field comment block (matching the existing annotation on [D3D12_VERTEX_FORMAT_COMPRESSED1_HEADER](#compressed1-position-encoding)).</li>
v0.26|5/6/2026|<li>Clarified in the [Clustered geometry](#clustered-geometry) (Cluster BLAS bullet, Cluster BLAS overview paragraph) and [Partitioned TLAS](#partitioned-top-level-acceleration-structures) sections that `CopyRaytracingAccelerationStructure` and `EmitRaytracingAccelerationStructurePostbuildInfo` are unsupported for these objects **except** for the serialize/deserialize and tools-visualization modes - `D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE`, [`_DESERIALIZE`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), and [`_VISUALIZATION_DECODE_FOR_TOOLS`](raytracing.md#d3d12_raytracing_acceleration_structure_copy_mode), plus the corresponding [`POSTBUILD_INFO_SERIALIZATION`](raytracing.md#d3d12_raytracing_acceleration_structure_postbuild_info_type) and `_TOOLS_VISUALIZATION` postbuild-info types used to size their outputs. These were already supported per the DXR1 spec but the DXR2 prose read as a blanket prohibition. Compaction and clone copy modes remain unsupported; `D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS` should be used to copy/move these objects.</li>
v0.27|5/14/2026|<li>In [D3D12_RTAS_CLUSTERED_GEOMETRY_FLAGS](#d3d12_rtas_clustered_geometry_flags), corrected the enum values to match the original design intent: `D3D12_RTAS_CLUSTERED_GEOMETRY_FLAG_OPAQUE = 0x80000000` (bit 31) and `D3D12_RTAS_CLUSTERED_GEOMETRY_FLAG_NO_DUPLICATE_ANYHIT_INVOCATION = 0x40000000` (bit 30) - previously the spec listed `0x40000000` and `0x20000000` respectively.  Clarified that the flag region uses only the top two bits, with bits 24-29 reserved (apps must zero, implementations must ignore) so that future spec revisions can either widen the geometry index past 24 bits or add more flags without committing to a particular split today.  Noted the bit ordering is inverted relative to DXR1's [D3D12_RAYTRACING_GEOMETRY_FLAGS](raytracing.md#d3d12_raytracing_geometry_flags).</li>
v0.28|5/17/2026|<li>For [Cluster BLAS](#clustered-geometry), clarified that `_SERIALIZE` and `_VISUALIZATION_DECODE_FOR_TOOLS` (and the corresponding postbuild sizing queries) require all referenced CLAS to be valid for the operation duration on the GPU timeline, while deserialization ordering semantics remain unchanged. Caveat added to both the Cluster BLAS overview bullet and the Cluster BLAS overview paragraph for consistency.</li><li>In [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info), for operation types other than [D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS](#d3d12_rtas_operation_type), clarified the formal relationship between `_GET_SIZES` and prebuild info: summed per-result `_GET_SIZES` is bounded by implicit prebuild `ResultDataMaxSizeInBytes`, and each individual `_GET_SIZES` result is bounded by any compatible prebuild query for that object (including compatible queries that differ only in batch-level count fields such as `MaxArgCount` and `MaxTotal*`-prefixed fields).</li><li>Clarified that for these same operation types, `ScratchDataSizeInBytes` may depend on batch-level totals and remains governed by each operation's existing prebuild compatibility rules (max/min bounds plus required exact matches).</li><li>Also clarified that `ResultDataMaxSizeInBytes` is always `0` when mode is [D3D12_RTAS_OPERATION_MODE_GET_SIZES](#d3d12_rtas_operation_mode), and is always `0` for [D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS](#d3d12_rtas_operation_type) regardless of mode.</li><li>Updated `GET_SIZES` mode wording to clarify that for non-move operations it is for per-result explicit-placement sizing, while implicit-destination sizing must use prebuild info; added a cross-reference to [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info) for the rationale (the implicit-layout-freedom that makes summed `_GET_SIZES` insufficient for implicit). Also fixed a pre-existing missing word in the surrounding paragraph (`functions VertexBuffer is null` -> `functions when VertexBuffer is null`).</li><li>Editorial consistency cleanup: replaced stale references to `ExecuteIndirectAccelerationStructureOperations()` and `GetRTASPrebuildInfo()` with canonical API names [ExecuteIndirectRTASOperations()](#executeindirectrtasoperations) and [GetRTASOperationPrebuildInfo()](#getrtasoperationprebuildinfo) where these names were still used in active prose.</li><li>In [D3D12_EXECUTE_INDIRECT_RTAS_OPERATIONS_FLAGS](#d3d12_execute_indirect_rtas_operations_flags), corrected the enum member name from `D3D12_RTAS_OPERATION_FLAG_NONE` to `D3D12_EXECUTE_INDIRECT_RTAS_OPERATIONS_FLAG_NONE` to avoid token collision with [D3D12_RTAS_OPERATION_FLAGS](#d3d12_rtas_operation_flags).</li><li>In [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info), corrected `D3D12_RTAS_TYPE_MOVE_CLUSTER_OBJECTS` to [D3D12_RTAS_OPERATION_TYPE_MOVE_CLUSTER_OBJECTS](#d3d12_rtas_operation_type).</li><li>In [D3D12_RTAS_CLAS_INPUTS_DESC](#d3d12_rtas_clas_inputs_desc), clarified prebuild compatibility wording to match other input descs: fields beginning with `Max` bound the operation, and all other fields must match exactly.</li><li>In [D3D12_RTAS_CLAS_INPUTS_DESC](#d3d12_rtas_clas_inputs_desc), corrected displayed type names in `MaxTotalClasCount`/`MaxClasCountPerArg` entries to [D3D12_RTAS_OPERATION_BUILD_BLAS_FROM_CLAS_ARGS](#d3d12_rtas_operation_build_blas_from_clas_args).</li><li>Fixed a broken markdown link in [Implicit vs explicit destinations for acceleration structure operations](#implicit-vs-explicit-destinations-for-acceleration-structure-operations) (`See also` in that section).</li>
v0.29|7/3/2026|<li>For `_VISUALIZATION_DECODE_FOR_TOOLS` results of Raytracing2 acceleration structure types, added `D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER` followed by the specialized operation input description before the existing visualization payload. The added data provides the operation-level build inputs needed to build from the returned visualization payload and preserves the existing trace-intersection and shader-visible behavior contract, without requiring preservation of the application's original build call, batching, memory placement, performance characteristics, operation mode, or exact source encodings.</li><li>Cluster Geometry visualization data uses fixed formats: the returned vertex format is always `D3D12_VERTEX_FORMAT_FLOAT32_3` and the returned index format is always `D3D12_INDEX_FORMAT_UINT32`, conveyed through the returned specialized input description.</li><li>Clarified that returned specialized input descriptions containing [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits) must provide `ClusterLimits` values sufficient for the returned visualization payload, without requiring them to match the application's original build values.</li>
v0.30|7/10/2026|<li>In [Compressed1 with cluster templates](#compressed1-with-cluster-templates), described why the preferred stored vertex format can differ between static CLAS builds and cluster template instantiation, and documented `TemplateInstancePrefersCOMPRESSED1` guidance. Linked the `VertexInstantiationFormat` member of [D3D12_RTAS_CLUSTER_TEMPLATE_TRIANGLES_INPUTS_DESC](#d3d12_rtas_cluster_template_triangles_inputs_desc) directly to the capability.</li><li>In [D3D12_RTAS_OPERATION_BUILD_CLUSTER_TEMPLATES_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_cluster_templates_from_triangles_args), made `InstantiationBoundingBoxLimit` independent of the format-specific argument union so the optional instantiation AABB applies to every template format, including `COMPRESSED1`. Defined finite, ordered bounds; undefined behavior for invalid bounds; the bounds as applying to source-format positions expressed as conceptual float32 values, with the implementation responsible for displacement introduced by instantiation-format conversion; and exact AABB matching between `GET_SIZES` and a subsequent explicit-destination build. The `Compressed1` union arm contains `Header` plus explicit reserved padding, establishing an eight-byte format-specific region without changing the GPU argument-array stride when future arms fit within it. Added flattened HLSL variants for `COMPRESSED1` and formats with no format-specific arguments.</li>
v0.31|8/6/2026|<li>For `_VISUALIZATION_DECODE_FOR_TOOLS` of [Cluster Geometry types](#clustered-geometry), clarified that the fixed returned vertex and index formats describe the encoding of the returned data only and do not relax any [D3D12_RTAS_CLUSTER_LIMITS](#d3d12_rtas_cluster_limits) maximum.  Since `MaxVertexCountPerCluster` is limited to a maximum supported value of 256, emitting one vertex per triangle corner is not valid for a cluster with enough triangles for that to exceed the limit.</li><li>Clarified the interaction between tools visualization of Cluster Geometry types and [degenerate primitives](raytracing.md#degenerate-primitives-and-instances): the allowance to replace the coordinates of discarded degenerate primitives with NaN during visualization applies to vertex positions only and does not relax the requirements for geometry indices and triangle flags.  Added a note that two related questions remain open: how geometry index and flags are defined for a primitive discarded from the build, and whether a returned NaN position is guaranteed to reproduce the same discard when a visualization result is used to rebuild.</li>
v0.32|8/8/2026|<li>In [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info), clarified for `ResultDataMaxSizeInBytes` that the `prebuild(A) + prebuild(B) <= prebuild(A+B)` inequality does not hold in the other direction. Summing the results of separate prebuild queries does not size a batch that spans them; a single query whose descriptor covers the whole batch is required.</li>
v0.33|8/13/2026|<li>Defined discarded primitive semantics for cluster builds. In [The role of geometry index in a cluster](#the-role-of-geometry-index-in-a-cluster), a primitive discarded during a cluster build or template instantiation still consumes its `PrimitiveIndex()`, so the indices of retained primitives are unaffected by a discard. In [D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER](#d3d12_rtas_tools_visualization_build_inputs_header), a discarded primitive is returned in place with degenerate index values, and a trailing run of discarded primitives may be omitted. The requirement that geometry indices and triangle flags flatten to the same value as the visualized structure is now scoped to retained primitives, and returned cluster vertex positions are stated to be finite, replacing the previous statement about the NaN allowance. Resolves the open items previously noted there.</li><li>In [D3D12_RTAS_OPERATION_PREBUILD_INFO](#d3d12_rtas_operation_prebuild_info), scoped the batch sizing guidance for `ResultDataMaxSizeInBytes` to implicit destinations, and stated that `D3D12_RTAS_OPERATION_MODE_EXPLICIT_DESTINATIONS` needs no batch-covering query since the field reports the size of a single result.</li>
v0.34|8/13/2026|<li>Corrected the code block for [D3D12_RTAS_OPERATION_DESC](#d3d12_rtas_operation_desc) to show its `Inputs` member inline, matching the API definition and the existing member descriptions. The code block previously showed a stale `pInputs` pointer, and three prose references to that name have been updated with it, including one that used pointer dereference syntax for what is an inline member.</li>
v0.35|8/26/2026|<li>Clarified `VertexCount` in [D3D12_RTAS_OPERATION_BUILD_CLAS_FROM_TRIANGLES_ARGS](#d3d12_rtas_operation_build_clas_from_triangles_args): the same definition applies when constructing a Cluster Template, where the count comes from the template's index buffer even though the vertex data is supplied at instantiation.  `VertexBuffer` is the only field whose optionality is added by the template case.</li>
v0.36|9/1/2026|<li>In [Clustered geometry](#clustered-geometry), noted that shaders compiled before Shader Model 6.9 cannot specify [RAYQUERY_FLAG_ALLOW_CLUSTERED_GEOMETRY](raytracing.md#rayquery-flags), since the template parameter carrying it was added in Shader Model 6.9, and pointed at the pipeline creation opt-in those shaders use instead.  See [RayQuery flags on pre Shader Model 6.9 shaders](raytracing.md#rayquery-flags-on-pre-shader-model-69-shaders).</li><li>Renamed `VertexCount`, `MaxVertexCountPerCluster`, and `MaxTotalVertexCount` in the Raytracing2 cluster APIs to `UniqueVertexIndexCount`, `MaxUniqueVertexIndexCountPerCluster`, and `MaxSumOfUniqueVertexIndexCounts`, respectively, to distinguish their unique-index semantics from DXR1 vertex-buffer counts. This is a source-only naming change; field sizes, ordering, and behavior are unchanged.</li>
v0.37|9/2/2026|<li>Defined cluster triangle participation and index-based relevant positions for direct builds and Cluster Template instantiation.  Template hints do not affect functional behavior, and non-participating triangles retain their `PrimitiveIndex()` values.</li><li>Applied relevance to per-position [COMPRESSED1](#compressed1-position-encoding) and `InstantiationBoundingBoxLimit` requirements, required finite uncompressed conversion results, and made source-compatible `COMPRESSED1` conversion deterministic.</li><li>Defined visualization equivalence for CLAS and Cluster Template instantiations, including duplicate-free any-hit and `RayQuery` behavior, and corrected `GET_SIZES` subset matching.</li><li>For tools visualization of CLAS, changed the fixed returned index format from `D3D12_INDEX_FORMAT_UINT32` to `D3D12_INDEX_FORMAT_UINT8`; implementations pack or remap returned vertices and indices as needed. Cluster Template visualization continues to return `D3D12_INDEX_FORMAT_UINT32` indices because template topology refers to vertex data supplied later during instantiation.</li>
v0.38|9/8/2026|<li>In [D3D12_RTAS_TOOLS_VISUALIZATION_BUILD_INPUTS_HEADER](#d3d12_rtas_tools_visualization_build_inputs_header), clarified tools-visualization pointer semantics: decoded input data and address arrays reside in the result buffer, while CLAS-address entries, instance BLAS references, and built Opacity Micromap array references remain external object references. Referenced objects are visualized separately.</li>
